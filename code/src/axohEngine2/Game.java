...
public abstract class Game extends JPanel implements Runnable {
	...
	protected Data data;
	protected Save save;
	// Variable that will hold all of the chestTiles within a specific range of
	// the player
	...
	// Game loop and Thread variable(Transient means it wont be serialized,
	// certain data types cant be serialized)
	private transient Thread gameloop;
	// Game lists to keep track of game specific data as well as their
	// accessible method counterparts
	...
	/***************************************************************
	 * Constructor - Initialize the frame, the backBuffer, the game lists, and
	 * any other variables making the frame of the game where the player moves.
	 ****************************************************************/
	...
	/*******************************************************
	 * @param btn
	 *            - Each mouse button is labeled with an Int, this number picks
	 *            that button
	 ...
	 *         <p>
	 *         Currently may not work, using built in java methods may work
	 *         better. Unused
	 ******************************************************/
	...
	/******************************************************
	 * @param g
	 *            - Graphics used to render objects Override the JFrames update
	 *            method and insert custom updating methods
	 ******************************************************/
	public void update(Graphics g) {
		// Make sure the game renders as fast as possible but only runs the
		// framerate amount of updates per second
		_frameCount++;
	...
	/******************************************************
	 * @param g
	 *            - The Systems Graphics object
	 *            <p>
	 *            Override the frames Paint method, draw the backBuffer and sync
	 *            with the system The purpose of this is to solve any strange
	 *            rendering glitches, doing it this way allows for an image to
	 *            be designed in the background and then brought forward all at
	 *            once.
	 ******************************************************/
	...
	// Using Runnable, run a loop which calls update methods for specific
	// actions including graphics and collisions
	public void run() {
		Thread t = Thread.currentThread();
		// Basically - While this new thread is equal to the thread we make at
		// startup, repeat
		while (t == gameloop) {
			try {
			...
			if (!gamePaused()) {
				...
				globalTileCollision();
				performCollision();
				// System.out.println("Number of sprites is : " +
				// _sprites.size() + "Name of sprite 1 " +
				// _sprites.getFirst()._name + " sprite 2 " +
				// _sprites.get(1)._name + " Sprite 3 " +
				// _sprites.getLast()._name);
			}
	...
	/**********************************************************************
	 * @return Data Get the current 'Data.java' class instance
	 *********************************************************************/
	public Data data() {
		return data;
	}
	...
	/**********************************************************************
	 * Mouse Listener events Inherited Method
	 *
	 * @param e
	 *            - A MouseEvent action which will change a number that
	 *            coordinates with having pressed that button
	 *********************************************************************/
	...
	/**********************************************************************
	 * @param e
	 *            -A MouseEvent that updates a mouses position after being
	 *            pressed Inherited Method
	 *********************************************************************/
	...
	/**********************************************************************
	 * Set the current key being pressed to the currentChar variable The purpose
	 * of this if for typeing on screen. Once the keycode's char is passed in,
	 * that currentChar variable can be accessed at that time for typing,
	 * instead of accessing hundreds of keycodes and chars.
	 *
	 * @param keyChar
	 *            - A char of what is being pressed down
	 *********************************************************************/
	...
	/**********************************************************************
	 * Return an angle of X or Y value based on a degree and return it in
	 * radians
	 *
	 * @param angle
	 *            - A Double from 0 to 360
		...
		for (AnimatedSprite spr : _sprites) {
			...
			if (state == STATE.GAME) {
				if (spr instanceof Mob) {
					((Mob) spr).updateMob(); // When the game is running, update
												// Mobs
	 ...
	 * <p>
	 * spr1 - The first sprite (Most of the time the player) spr2 - The second
	 * sprite (Most of the time a random npc or enemy) hitDir - The bound which
	 * is being intersected on spr1 hitDir2 - The bound which is being
	 * intersected on spr2
	 **************************************************************************/
	protected void spriteCollision() {
		for (int i = 0; i < _sprites.size(); i++) {
			if (Objects.equals(_sprites.get(i)._name, "mainC"))
			...
			for (AnimatedSprite _sprite : _sprites) {
				...
				// hasMultBounds - Set to true if there is more than one bound
				// being used
				...
				if (!spr1hasMultBounds && !spr2hasMultBounds) {
					// Determines whether sprite 1 is currently intersecting
					// with sprite 2
					if (spr1GetBounds.intersects(spr2GetBounds))
						spriteCollision(spr1, _sprite, -1, -1); // spr1 and spr2
																// have one
																// bound
				} else {
					// If sprite 1 has more then one bound being used, but no
					// bounds are being used on sprite 2
					if (spr1hasMultBounds && !spr2hasMultBounds) {
						// Checks if the leftBound is being intersected by
						// sprite 2
						if (spr1.checkLeftBound(spr2GetBounds))
							spriteCollision(spr1, _sprite, 0, -1);

						// Checks if the leftBound is being intersected by
						// sprite 2
						if (spr1.checkRightBound(spr2GetBounds))
							spriteCollision(spr1, _sprite, 1, -1);

						// Checks if the leftBound is being intersected by
						// sprite 2
						if (spr1.checkHeadBound(spr2GetBounds))
							spriteCollision(spr1, _sprite, 2, -1);

						// Checks if the leftBound is being intersected by
						// sprite 2
						if (spr1.checkLegBound(spr2GetBounds))
							spriteCollision(spr1, _sprite, 3, -1);
					}

					// If sprite 2 has more then one bound being used, but no
					// bounds are being used for sprite 1
					if (spr2hasMultBounds && !spr1hasMultBounds) {
						// spr2 has multiple bounds but not spr1
						if (_sprite.checkLeftBound(spr1GetBounds))
							spriteCollision(spr1, _sprite, -1, 0);

						if (_sprite.checkRightBound(spr1GetBounds))
							spriteCollision(spr1, _sprite, -1, 1);

						if (_sprite.checkHeadBound(spr1GetBounds))
							spriteCollision(spr1, _sprite, -1, 2);

						if (_sprite.checkLegBound(spr1GetBounds))
							spriteCollision(spr1, _sprite, -1, 3);

					}
					if (spr2hasMultBounds && spr1hasMultBounds) { // spr2 has
																	// multiple
																	// bounds as
																	// well as
																	// spr1
	...
	/**********************************************************************
	 * Same as the above spriteCollision() method but instead the collision is
	 * between a sprite and a Tile. Also, currently only with rectangles.
	 * <p>
	 * The method gets a sprite and then gets each tile, if either objects
	 * intersects any bounds made for either object the method calls a handling
	 * method for dealing with very specific properties that are relative to
	 * each game
	 ***********************************************************************/
	...
	// Draw animated sprites automatically, they must be in the list (Includes
	// tiles)
	...
	/*********************************************************************
	 * @param tile
	 *            - A Tile to be added in to the system
	 *            <p>
	 *            Instead of just adding all of the tiles in a Map to the system
	 *            for updating, use this method to add a layer of
	 *            choice(filter). This method currently only allows Tiles which
	 *            have properties - solid, event, breakable, etc..
	 *            <p>
	 *            The purpose of this is because there could be thousands of
	 *            tiles in a single map not all of these tiles have properties
	 *            that need updateing like animations. This allows for a much
	 *            faster, smoother game experience as well as larger maps.
	 *********************************************************************/
	...
	/**********************************************************************
	 * @param g2d
	 *            - Gaphics used to display to the JFrame
	 * @param text
	 *            - The String of text to alter
	 * @param x
	 *            - An Int position relating to the X position the text will be
	 *            rendered on screen
	 **********************************************************************/
