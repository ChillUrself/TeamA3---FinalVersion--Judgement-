...
public class Mob extends AnimatedSprite implements MouseListener {
	...
	// health - HP
	/*
	 * ai - An enum which sets the ai of an npc, that ai needs to be written in
	 * a method and then added as a TYPE in the TYPE.java class before it can be
	 * used. Right now only random ai is possible, and PLAYER.
	 */
	...
	// attacking - A possible state the mob could be in, for many kinds of
	// checks
	// takenOut - Boolean to see if the mob has it's weapon out
	// currentAttack - The currently selected attack to use from the list of Mob
	// attacks
	...
	// Four variable booleans depicting the last direction the mob was
	// moving(This could be phased out of the system)
	 ...
	 *
	 * @param panel
	 *            - JFrame window for display
	 * @param g2d
	 *            - Graphics2D object used for displaying an image on a JFrame
	 * @param sheet
	 *            - The spriteSheet the animation or image is taken from
	 * @param spriteNumber
	 *            - The position on the spriteSheet the animation is or image
	 *            starts
	 * @param ai
	 *            - A TYPE enum depicting the ai's kind (radom, set path,
	 *            player, etc)
	 * @param name
	 *            - The character name in a String
	 * @param hostility
	 *            - Boolean, is the mob going to attack the player?
	 *************************************************************************/
	public Mob(Judgement panel, Graphics2D g2d, SpriteSheet sheet, int spriteNumber, TYPE ai, String name,
			boolean hostility) {
	...
	/***************************************************************
	 * Stop a mob from moving. This stop depends on the type of ai, this needs
	 * to be updated every time a new ai is added. This does not work for player
	 * mobs.
	 ****************************************************************/
	public void stop() {
	...
	/***************************************************************
	 * Method used to change a mobs position by the xa and ya parameters. This
	 * also updates a mobs aniamtion based on what direction is is moving in.
	 * Four animations are needed for a full moving sprite.
	 *
	 * @param xa
	 *            - Int movement in pixels on the x axis
	 * @param ya
	 *            - Int movement in pixels on the y axis
	 ****************************************************************/
	...
	/********************************************************************************
	 * Update various player related information. Methods like these can be used
	 * for other AI's as well. Update: weapon in/out, direction, keys pressed,
	 * movements etc.
	 *
	 * @param left
	 *            - keyCode
		...
		if (left) {
			...
			if (wasLeft && !up && !down && !right) {
				...
				if (!takenOut)
					setAnimTo(leftAnim);
				if (takenOut)
					setAnimTo(currentAttack.getMoveAnim(direction));
			}
			...
			if (moveDir != DIRECTION.LEFT) {
				if (!takenOut)
					setAnimTo(leftAnim);
				if (takenOut)
					setAnimTo(currentAttack.getMoveAnim(DIRECTION.LEFT));
				if (hasMultBounds) {
					toggleLeg(true);
					toggleLeft(false);
					toggleRight(false);
		...
		if (right) {
			...
			if (moveDir != DIRECTION.RIGHT) {
				if (!takenOut)
					setAnimTo(rightAnim);
				if (takenOut)
					setAnimTo(currentAttack.getMoveAnim(DIRECTION.RIGHT));
				if (hasMultBounds) {
					toggleLeg(true);
					toggleLeft(false);
					toggleRight(false);
		...
		if (up) {
			...
			if (moveDir != DIRECTION.UP) {
				if (!takenOut)
					setAnimTo(upAnim);
				if (takenOut)
					setAnimTo(currentAttack.getMoveAnim(DIRECTION.UP));
				if (hasMultBounds) {
					toggleLeg(false);
					toggleLeft(true);
					toggleRight(true);
		...
		if (down) {
			...
			if (moveDir != DIRECTION.DOWN) {
				if (!takenOut)
					setAnimTo(downAnim);
				if (takenOut)
					setAnimTo(currentAttack.getMoveAnim(DIRECTION.DOWN));
				if (hasMultBounds) {
					toggleLeg(false);
					toggleLeft(true);
					toggleRight(true);
	...
	/*********************************************************************************
	 * Check to see if a mob is currently attacking or change the state of
	 * whether it is attacking or not. Depnding on the check, certain animations
	 * can be played out.
	 *********************************************************************************/
	public void inOutItem() {
		...
		if (takenOut)
			playOnce(currentAttack.getMoveAnim(direction), currentAttack.getMoveTotal(), currentAttack.getMoveDelay(),
					inOutAnim + inOutTotal);
		else {
			if (direction == DIRECTION.LEFT)
				playOnce(leftAnim, walkFrames, walkDelay, inOutAnim + inOutTotal);
			if (direction == DIRECTION.RIGHT)
				playOnce(rightAnim, walkFrames, walkDelay, inOutAnim + inOutTotal);
			if (direction == DIRECTION.UP)
				playOnce(upAnim, walkFrames, walkDelay, inOutAnim + inOutTotal);
			if (direction == DIRECTION.DOWN)
				playOnce(downAnim, walkFrames, walkDelay, inOutAnim + inOutTotal);
		}
	}
	...
	// Set the animations for attacks of whatever attack is selected and play
	// them on screen
	public void attack() {
		attacking = true;
		setFullAnim(currentAttack.getAttackAnim(direction), currentAttack.getAttackTotal(),
				currentAttack.getAttackDelay());
		playOnce(currentAttack.getMoveAnim(direction), currentAttack.getMoveTotal(), currentAttack.getMoveDelay(),
				currentAttack.getAttackAnim(direction) + currentAttack.getAttackTotal());
	}
	 ...
	 *
	 * @param name
	 *            - String differentiating one attack from another
	 * @param magicDam
	 *            - Number to be used when calculating attack Magic damage
	 * @param strengthDam
	 *            - Number to be used when calculating attack Strength damage
	 *********************************************************/
	...
	/*******************************************************************
	 * @param name
	 *            - String name of the attack to retrieve from the Mob
	...
	/***************************************************
	 * @param name
	 *            - String name of the attack for the Mob to use now
	 ***************************************************/
	...
	/****************************************************
	 * Lower this mobs health by a damage as well by a modifier from 0 to a
	 * random int of a maximum value of the damage parameter % 5.
	 *
	 * @param damage
	...
	/***************************************************
	 * Get the x or y location of the mob in the room or set a new x or y
	 * location relative to it's current position
	 *
