...
public class Judgement extends Game implements ActionListener {
	...
	//this gets drawn and updated each refresh
	private String objective = "Defeat the enemy!";
	/*----------- Menus ----------------
	...
	private boolean keyLeft, keyRun, keyRight, keyUp, keyDown, keyAction, keyBack, keyEnter, keySpace, keyEscape,
			keyAttack, keyHealthUp, keyHealthDown, keyMagicUp, keyMagicDown, keyFPS, keyUI;
	private STATE state;
	...
	*/
	...
	private boolean gameMusicPlaying = false; // used in gamerefresh check (line
												// 426) to keep the refresh from
												// playing the song more than
												// once.
	...
	// gets the screen width and height so the game will fit on the proper
	// screen
	...
	/****************************************************************************
	 * Inherited Method This method is called only once by the 'Game.java'
	 * class, for startup Initialize all non-int variables here
	 *****************************************************************************/
	void gameStartUp() {
		...
		for (int i = 0; i < currentMap.getHeight() * currentMap.getHeight(); i++) {
			...
			// Check if the currentMap overlay has a mob object at that tile
			// location
	...
	private void initVariables() {
		...
		playerMob.setHealth(35); // If you change the starting max health, dont
									// forget to change it in inGameMenu.java
									// max health also
	...
	/****************************************************************************
	 * Inherited Method Method that updates with the default 'Game.java' loop
	 * method Add game specific elements that need updating here
	 *****************************************************************************/
	void gameTimedUpdate() {
		...
		if (state == STATE.INGAMEMENU) {
			inMenu.update(option, sectionLoc, playerMob.health()); // In Game
																	// Menu
																	// update
		}
	}
	...
	/**
	 * Inherited Method Obtain the 'graphics' passed down by the super class
	 * 'Game.java' and render objects on the screen
	 */
	...
	void gameRefreshScreen() {
		...
		if (state == STATE.INGAMEMENU) {
			...
			if (confirmUse)
				g2d.drawString("Use this?", CENTERX, CENTERY);
		}
	...
	private void refreshGameScreen(int maxHealth) {
		if (state == STATE.GAME) {
			...
			// System.out.println("CenterX: " + (CENTERX - playerX) + " CenterY:
			// " + (CENTERY - playerY));
			// Render the map, the player, any NPCs or Monsters and the player
			// health or status
...
//			System.out.println(swordCollision());
			...
			// if the toggle is true then the screen sets up to what it needs to
			// be to play the game.
			if (uiToggle == true) {
				...
				// GradientPaint gp2 = new GradientPaint(5, 50, Color.DARK_GRAY,
				// 5, 5, Color.WHITE, true);
				...
				g2d.setColor(Color.black);
				g2d.fillRect((int) (CENTERX / 8.42), (int) (CENTERY / 12.27), (int) (SCREENWIDTH / 10.97),
						SCREENHEIGHT / 36);
				g2d.setColor(Color.RED);
				g2d.fillRect((int) (CENTERX / 8.42), (int) (CENTERY / 13.5), (int) (SCREENWIDTH / 10.97),
						SCREENHEIGHT / 36);
				g2d.setPaint(healthGradient);
				g2d.fillRect((int) (CENTERX / 8.42), (int) (CENTERY / 13.5), SCREENWIDTH * maxHealth / 384,
						SCREENHEIGHT / 36);
				g2d.setColor(Color.BLACK);
				...
				g2d.setColor(Color.BLACK);
				g2d.fillRect((int) (CENTERX / 5.4), (int) (CENTERY / 6.1), (int) (SCREENWIDTH / 10.7),
						SCREENHEIGHT / 36);
				g2d.setColor(DARKBLUE);
				g2d.fillRect((int) (CENTERX / 5.49), (int) (CENTERY / 6.35), (int) (SCREENWIDTH / 10.7),
						SCREENHEIGHT / 36);
				g2d.setPaint(manaGradient);
				g2d.fillRect((int) (CENTERX / 5.49), (int) (CENTERY / 6.35), SCREENWIDTH * inMenu.getMagic() / 384,
						SCREENHEIGHT / 36);
				g2d.setColor(Color.BLACK);
				...
				//objective indicator
				//use setObjective method to update
				g2d.drawString("Current Objective:" + objective, 30, 120);
				
				// g2d.drawString(""+lastCollided, CENTERX, CENTERY+300);

				/*
				 * g2d.setColor(Color.BLACK); g2d.drawString("Player: " +
				 * playerName, CENTERX - (int)(898/screenWidthRatio()), CENTERY
				 * +(int)(484/screenHeightRatio())); g2d.setColor(Color.WHITE);
				 * g2d.drawString("Player: " + playerName, CENTERX -
				 * (int)(900/screenWidthRatio()), CENTERY
				 * +(int)(482/screenHeightRatio())); g2d.drawString("X:" +
				 * perfectX, CENTERX + (int)(600/screenWidthRatio()), CENTERY
				 * +(int)(482/screenHeightRatio())); g2d.drawString("Y:" +
				 * perfectY, CENTERX + (int)(750/screenWidthRatio()), CENTERY
				 * +(int)(482/screenHeightRatio())); g2d.setColor(Color.CYAN);
				 */
				// g2d.drawRect(playerMob.getBounds().x,
				// playerMob.getBounds().y, playerMob.getBounds().width,
				// playerMob.getBounds().height);
				// g2d.setColor(Color.PINK);
				/*
				 * g2d.drawRect(playerMob.getLeftBound().x,
				 * playerMob.getLeftBound().y, playerMob.getLeftBound().width,
				 * playerMob.getLeftBound().height);
				 * g2d.setColor(Color.MAGENTA);
				 * g2d.drawRect(playerMob.getRightBound().x,
				 * playerMob.getRightBound().y, playerMob.getRightBound().width,
				 * playerMob.getRightBound().height); g2d.setColor(Color.RED);
				 * g2d.drawRect(playerMob.getTopBound().x,
				 * playerMob.getTopBound().y, playerMob.getTopBound().width,
				 * playerMob.getTopBound().height); g2d.setColor(Color.YELLOW);
				 * g2d.drawRect(playerMob.getBottomBound().x,
				 * playerMob.getBottomBound().y,
				 * playerMob.getBottomBound().width,
				 * playerMob.getBottomBound().height);
				 */
				// g2d.setColor(Color.WHITE);
				// g2d.drawRect(playerMob.getSwordBound().x,
				// playerMob.getSwordBound().y, playerMob.getSwordBound().width,
				// playerMob.getSwordBound().height);
				g2d.setColor(Color.WHITE);
				// g2d.drawString("," +currentOverlay.currentMobs[0].getYLoc(),
				// CENTERX + 870, CENTERY +450);
				...
				for (int i = 0; i < currentOverlay.currentMobs.length; i++) {
					if (currentOverlay.currentMobs[i] != null) {
						...
						// g2d.drawRect(npc.getBounds().x, npc.getBounds().y,
						...
						if (drawingDamage == true) {
							...
							if (lastDamage < 10) {
								g2d.setColor(Color.BLACK);
								g2d.drawString("" + lastDamage, (int) npc.getXLoc() + 54,
										(int) npc.getYLoc() + 52 - damageTimer);
								g2d.setColor(Color.WHITE);
								g2d.drawString("" + lastDamage, (int) npc.getXLoc() + 52,
										(int) npc.getYLoc() + 50 - damageTimer);
							} else if (lastDamage >= 10 && lastDamage < 100) {
								g2d.setColor(Color.BLACK);
								g2d.drawString("" + lastDamage, (int) npc.getXLoc() + 44,
										(int) npc.getYLoc() + 52 - damageTimer);
								g2d.setColor(Color.WHITE);
								g2d.drawString("" + lastDamage, (int) npc.getXLoc() + 42,
										(int) npc.getYLoc() + 50 - damageTimer);
							}
							g2d.setComposite(d);
						}
				...
				if (damageTimer >= 40) {
					...
					// g2d.clearRect(50, 50,
					// (int)currentOverlay.accessTile(300).mob().getXLoc(),
					// (int)currentOverlay.accessTile(300).mob().getYLoc());
				}
			}
		}
	}
	...
	/*******************************************************************
	 * The next four methods are inherited Currently these methods are not being
	 * used, but they have been set up to go off at specific times in a game as
	 * events. Actions that need to be done during these times can be added
	 * here.
	 ******************************************************************/
	void gameShutDown() {
	}
	...
	void spriteCollision(AnimatedSprite spr1, AnimatedSprite spr2, int hitDir, int hitDir2) {
		// Only check for sprite collision if the user is not in the title
		// screen
		if (state != STATE.TITLE) {
			/*
			 * System.out.println("Sprite Collision Being Called");
			 * 
			 * //Get the smallest possible overlap between the two problem
			 * sprites int sp1BoundSize = spr1.getBoundSize(); double spr1BoundX
			 * = spr1.getBoundX(hitDir); double spr1BoundY =
			 * spr1.getBoundY(hitDir);
			 * 
			 * int sp2BoundSize = spr2.getBoundSize(); double spr2BoundX =
			 * spr2.getBoundX(hitDir2); double spr2BoundY =
			 * spr2.getBoundY(hitDir2);
			 * 
			 ...
			 * double smallestOverlap = Double.MAX_VALUE; shiftX = 0; shiftY =
			 * 0;
			 * 
			 ...
			 * 
			 * checkSpriteSpriteOverlap(leftOverlap, rightOverlap, topOverlap,
			 * botOverlap, smallestOverlap);
			 * 
			 * //Handling very specific collisions if (spr1.spriteType() ==
			 * TYPE.PLAYER && state == STATE.GAME) { if (spr2 instanceof Mob)
			 * ((Mob) spr2).stop();
			 */

			// This piece of code is commented out because I still need the
			// capability of getting a tile from an xand y position
			/*
			 * if(((Mob) spr1).attacking() && currentOverlay.getFrontTile((Mob)
			 * spr1, playerX, playerY, CENTERX,
			 * CENTERY).getBounds().intersects(spr2.getBounds())){ ((Mob)
			 * spr2).takeDamage(25); // }
			 */

			// Handle simple push back collision
			/*
			 * if (playerX != 0) playerX -= shiftX; if (playerY != 0) playerY -=
			 * shiftY; if (playerX == 0) setMapX((int) (getMapX() - shiftX)); if
			 * (playerY == 0) setMapY((int) (getMapY() - shiftY)); }
			 */
		}
	}
		...
		if (state != STATE.TITLE && spr.collided() == true) {
			...
			if (spr.getTopBound().intersects(tile.getTileBounds())) {
				if (keyAction == true && (tile._name).equals("chest")) {
					tile.setFrame(tile.getSpriteNumber() + 1); // Chests should
																// have opened
																// and closed
																// version next
																// to each other
					interaction.play();
					// inMenu.addItem(tile.event().getItem()); //Add item to
					// inventory
				}
			}
		}
	...
	/**********************************************************
	 * The Depths of Judgement Lies Below The movement of the player, if the
	 * player can move left or right, up or down, collision into any walls or
	 * players
	 * <p>
	...
	private void checkGame(int xa, int ya) {
		/********************************************
		 * Special actions for In Game Defines what happens when the keys are
		 * pressed
		 *******************************************/
	...
	private void checkMenu() {
		...
		if (option == OPTION.ITEMS) {
			...
			if (keyDown) {
				if (sectionLoc == 3)
					inMenu.loadNextItems();
				if (inMenu.getTotalItems() > sectionLoc + 1 && sectionLoc < 3)
					sectionLoc++;
				inputWait = 8;
			}
			// Enter key(Make a choice)
			if (keyEnter) {
				...
				if (inMenu.checkCount() > 0 && keyEnter)
					confirmUse = true;
				inputWait = 10;
			}
		...
		if (option == OPTION.EQUIPMENT) {
			...
			if (keyDown) {
				if (sectionLoc == 3)
					inMenu.loadNextEquipment();
				if (inMenu.getTotalEquipment() > sectionLoc + 1 && sectionLoc < 3)
					sectionLoc++;
				inputWait = 8;
			}
		}
		...
		if (keyBack && option == OPTION.NONE) {
			state = STATE.GAME;
			super.setGameState(STATE.GAME); // actually change the state of the
											// Game.java class so that mobs can
											// move
			option = OPTION.NONE;
			inLocation = 0;
			sectionLoc = 0;
	...
	/*
	 * public void RenderBoundsInGame() { //Add the tiles from the map to be
	 * updated each system cycle for (int i = 0; i < currentMap.getHeight() *
	 * currentMap.getHeight(); i++) {
	 * 
	 * Tile accessTile = currentMap.accessTile(i); if (accessTile.isSolid()) {
	 * accessTile.drawBounds(Color.red); } // Check if the currentMap overlay
	 * has a mob object at that tile location
	 * 
	...
	/**
	 * Inherited method Currently unused
	 */
	void gameMouseDown() {
	}

	// From the title screen, load a game file by having the super class get the
	// data,
	// then handling where the pieces of the data will be assigned here.

	/**
	 * Inherited method Currently if the game is running and the sword is out,
	 * the player attacks with it
	 */
	void gameMouseUp() {
	...
	/*
	 * Used by the Timer every 30 seconds to update the Data class with the
	 * current variables
	 */
	@Override
	...
	public void setCurrentMap(Map m) {
		currentMap = m;
		System.out.println(currentMap._name);
	}

	public void setCurrentOverlay(Map o) {
		currentOverlay = o;
		System.out.println(currentOverlay._name);
	}
